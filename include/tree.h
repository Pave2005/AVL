...
class SearchTree {
   private:
    struct Node {
        ...
        Node(Node &&rhs) noexcept { this->swap(rhs); }
   ...
   public:
    ...
    SearchTree(const SearchTree &rhs) : root_(nullptr) {
        ...
        while (!queue.empty()) {
            ...
            if (node->left_) {
                std::unique_ptr<Node> tmp_node_left_unique =
                    std::make_unique<Node>((node->left_)->key_);
            ...
            if (node->right_) {
                std::unique_ptr<Node> tmp_node_right_unique =
                    std::make_unique<Node>((node->right_)->key_);
    ...
    // -----
    SearchTree(SearchTree &&rhs) noexcept { this->swap(rhs); }
   ...
   private:
    ...
    Node *create_node(Args &&...key) {
        ...
        first_elem_ = (!first_elem_) ? new_node.get() : first_elem_;
        first_elem_ =
            (key_compare(new_node->key_, first_elem_->key_)) ? new_node.get() : first_elem_;
    ...
    void add_new_node(const T &key, Node *&new_node, Node *curr_node_ptr) {
        ...
        while ((left_less = key_compare(key, curr_node_ptr->key_)) && curr_node_ptr->left_ ||
            ...
            if (left_less)
                curr_node_ptr = curr_node_ptr->left_;
            else
                curr_node_ptr = curr_node_ptr->right_;
        }
   ...
   public:
    ...
    void node_dump(std::ofstream &file) const {
        ...
        while (!stack.empty()) {
            ...
            file << node->indx_
                 << "[shape = doubleoctagon, style = filled, fillcolor = cornflowerblue "
                    "label = \""
                 << node->key_ << "\"]" << std::endl;

            if (!is_root)
                file << node->parent_->indx_ << " -> " << node->indx_ << std::endl;
            else
                is_root = false;
